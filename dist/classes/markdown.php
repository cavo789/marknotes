...
* @package   : 2017-02-03T17:51:33.904Z
*/
?>
<?php
...
class aeSecureMarkdown
{
        ...
        foreach ($arr as $fname) {
            ...
            if (aeSecureFiles::fileExists($fname)) {
                        ...
                        if (trim($tmp[$name])!='') { // can't be empty
                            ...
                            if (preg_match('/^[A-Za-z0-9-_\.]+$/', trim($tmp[$name]))) { // should only contains letters, figures or dot/minus/underscore
                                ...
                                if (is_file(dirname(__DIR__).'/templates/'.trim($tmp[$name]).'.php')) {
                                    ...
                                    if ($this->_DEBUGMODE) {
                                        echo '<span style="font-size:0.8em;">'.__FILE__.'::'.__LINE__.'</span>&nbsp;-&nbsp;';
                                    }
                ...
                if (isset($this->_json['password'])) {
                    $this->_settingsPassword=$this->_json['password'];
                }
                ...
                if (isset($this->_json['page'])) {
                    ...
                    if (isset($tmp['google_font'])) {
                        $this->_settingsFontName=str_replace(' ', '+', $tmp['google_font']);
                    }
                    if (isset($tmp['img_maxwidth'])) {
                        $this->_settingsImgMaxWidth=str_replace(' ', '+', $tmp['img_maxwidth']);
                    }
                }
                ...
                if (isset($this->_json['encryption'])) {
                    ...
                    if (isset($tmp['password'])) {
                        $this->_encryptionPassword=$tmp['password'];
                    }
                    if (isset($tmp['method'])) {
                        $this->_encryptionMethod=$tmp['method'];
                    }
                }
                ...
                if (isset($this->_json['optimisation'])) {
                    ...
                    if (isset($tmp['cache'])) {
                        $this->_useCache=(($tmp['cache']==1)?true:false);
                    }
                    if (isset($tmp['lazyload'])) {
                        $this->_OptimizeLazyLoad=(($tmp['lazyload']==1)?1:0);
                    }
                }
    ...
    */
    private function jstree_hide_emptyFolder(&$array)
    {
        ...
        foreach ($array as &$value) {
            if (is_array($value)) {
                $this->jstree_hide_emptyFolder($value, 'children');
            }
        }
    ...
    */
    private function dir_to_jstree_array(string $dir, string $order = "a", array $ext = array()) : array
    {
        ...
        if ($opened==false) {
            if (in_array(rtrim(utf8_encode($dir), DS), $this->_settingsFoldersAutoOpen)) {
                $opened=true;
            }
        }
    ...
    */
    private function GoogleFont() : string
    {
        ...
        if ($this->_settingsFontName!=='') {
            ...
            for ($i=1; $i<7;
            $i++) {
                $result.='page h'.$i.'{font-family:"'.$sFontName.'";}';
            }
            $result.='</style>';
        } // if ($this->_settingsFontName!=='')
