...
jQuery.fn.albeTimeline = function (json, options) {
	...
	var eMenu = $("<ul>")
		.attr("id", "timeline-menu");
	var eTimeline = $("<section>")
		.attr("id", "timeline");

	$.each(json, function (index, element) {

		var ano = new Date(element.time)
			.getFullYear();
		var separador = $(eTimeline)
			.find("div.group" + ano);
	...
	var ePanel = $("<div>")
		.addClass("panel")
		.append(badge);
	eTimeline.append($("<article>")
		.append(ePanel));
	eTimeline.append($("<div>")
		.addClass("clearfix")
		.css({
			"float": "none"
		}));
	...
	$.each(eTimeline.find("article"), function (index, value) {
		...
		$(this)
			.addClass((index % 2 == 0) ? "" : "inverted");
		//Adiciona classe de animação.
		if (settings.effect && settings.effect != 'none')
			$(this)
			.addClass("animated " + settings.effect);
	});
