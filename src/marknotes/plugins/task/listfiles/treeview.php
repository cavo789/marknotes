 ...
 * in the JSON answer
 *
 * Answer to /index.php?task=task.listfiles.treeview or the "fake" file /listfiles.json
 */
    ...
    */
    private static function makeJSON(string $dir) : array
    {
        ...
        **/
        ...
        $listDir = array
        (
            'id' => str_replace(DS, '/', utf8_encode(str_replace($root, '', $dir).DS)),
            'type' => 'folder',
            'icon' => 'folder',
        ...
        foreach ($arrEntries as $entry) {
            if ($entry['type'] == 'file') {
                ...
                if (strlen($sFileText) > 30) {
                    $sFileText = substr($sFileText, 0, 30).'…';
                }
                ...
                $files[] = array(
                    ...
                    'data' => array(
                        //'task' => 'display',
                        'basename' => utf8_encode($aeFiles->removeExtension(basename($dataURL))),
                        'file' => utf8_encode($dataFile),
                        'url' => utf8_encode($dataURL)
                    ),
                    'state' => array(
                ...
                // acls plugin options
                //
                // See function run() of MarkNotes\Plugins\Task\ACLs\cansee.php
    ...
    public static function run(&$params = null) : bool
    {
        ...
        if (!$show_tree_allowed) {
            ...
            if ($sReturn === '') {
                ...
                // Build the json
                $return = array();
                //$return['settings'] = array('root' => str_replace(DS, '/', $docs));
                $return['count'] = $count;
                $return['tree'] = $arr;
